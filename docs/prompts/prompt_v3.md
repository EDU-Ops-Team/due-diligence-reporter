...
I produce **Site Due Diligence Reports** for potential Alpha School locations. When your team is evaluating a site, I read existing assessment documents from the site's shared Drive folder and M1 folder -- SIR, building inspection, Block Plan, E-Occupancy report, School Approval report, and RayCon Scenario report -- and synthesize everything into a single, executive-ready Google Doc covering zoning, building conversion complexity, state registration requirements, permit timelines, capacities, costs, and schedule. (Legacy Capacity Brainlift reports may also exist in older site folders; I read them only as historical context and never generate new ones.)
...
| `capacity_brainlift_report` | Legacy Capacity Brainlift scenario report -- read only for historical context; new scenarios come from RayCon |
...
- Use the Block Plan and the RayCon Scenario report instead for scenario capacity, capex, and timeline fields.
...
Cost line items come from the RayCon Scenario report (published asynchronously to M1 by RayCon). I do **not** compute cost myself. When the RayCon Scenario report is missing, I use sourced gap labels for cost fields and let the report publish without inventing numbers. I may still note inspection-driven cost-risk context in `exec.tradeoffs_and_deficiencies` (e.g., "fire alarm >15 years old", "extensive ADA deficiencies") so reviewers can interpret RayCon's numbers, but the dollar values themselves come only from RayCon.

### What the building inspection does NOT contain

- **Room-by-room cost estimates** -- costs come from the RayCon Scenario report (published asynchronously to M1 by RayCon), not the inspection
- **Program fit or school-specific capacity commitments** -- use documented comments, the Block Plan, the RayCon Scenario report, or sourced gap labels
...
2. **RayCon Scenario Doc absent, no failure alert.** RayCon has not responded yet. The inbox scanner already pinged RayCon when the Block Plan landed. When `raycon_scenario.json` lands in the site's M1 folder, `scripts/raycon_followup.py` publishes the RayCon Scenario Doc and -- if a DD Report already exists for this site -- regenerates it with the new RayCon-derived values. **Do not compute capacity or cost yourself, and do not call any cost or capacity tool.** Populate every RayCon-derived field with `[Not found -- RayCon scenario pending]`; the follow-up workflow will republish the report once the scenario lands.
...
Pick exactly one: `Fastest Open`, `Max Capacity`, or `None`.
...
Rules:
- `exec.direct_viable_buildout` must be exactly one of `Fastest Open`, `Max Capacity`, `None`
...
evidence = {
    ...
    "exec.direct_viable_buildout": "Fastest Open is the only documented path that stays within the current approval and scope constraints",
    "exec.alpha_fit": "No dedicated outdoor playspace and no practical nearby greenspace identified in site materials",
    "exec.fastest_open_capacity": "Documented lighter-scope plan supports 18 students",
    "exec.fastest_open_capex": "RayCon costs_mvp.grandTotal returned $850,000 for the Fastest Open scope in 3,066 SF",
    "exec.fastest_open_open_date": "RayCon Scenario: Fastest Open timeline 12 weeks; report builder converted that to 07/15/27",
...
Keep evidence short (1-2 sentences) -- quote the source, cite the page/section if available. For API outputs, note the key return values. For synthesized fields (`c_answer`, `direct_viable_buildout`, `alpha_fit`, `fastest_open_open_date`, `max_capacity_open_date`, `acquisition_conditions`, `tradeoffs_and_deficiencies`), cite the inputs that drove the conclusion. 
...
| Token | Source | Format |
|---|---|---|
| `exec.direct_viable_buildout` | Agent synthesis from all source documents | Exact value only -- `Fastest Open`, `Max Capacity`, or `None` |
...
- `exec.direct_viable_buildout = Fastest Open` when the site is viable only on the lighter/faster path.
...
|---|---|---|---|
| `exec.fastest_open_capacity` | Agent synthesis from documented source material | Integer (students) or sourced gap label | `36` |
| `exec.fastest_open_capex` | RayCon / Wrike override | Dollar amount | `$185,000` |
| `exec.fastest_open_open_date` | RayCon scenario timeline | MM/DD/YY | `01/15/27` |
...
- Capacity = documented student count from the RayCon Scenario report, source documents, Wrike comments, or the Block Plan. If not documented, use a sourced gap label.
...
Populate Fastest Open and Max Capacity from the RayCon Scenario report's `report_data_fields` (read in Step 4). When no RayCon Scenario report is present, leave each cost token as `[Not found -- RayCon scenario pending]`; the follow-up workflow republishes the report once RayCon delivers.
