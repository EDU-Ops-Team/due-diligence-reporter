...
The read-only `portfolio_automation_gap_snapshot` MCP tool rolls those Rhodes records up across active sites. It reads Rhodes-linked Drive folder status, registered DD source documents, `AutomationEvent v1` notes, pending automation review tasks, and P1 DRI assignment status, then returns per-site gap reasons plus portfolio totals. It does not write to Rhodes, Drive, Gmail, or Chat. Operators can run the same check with `uv run ddr portfolio-gaps`; the `Portfolio Automation Gaps` workflow runs the check on weekdays and stores both a text summary and JSON artifact.
